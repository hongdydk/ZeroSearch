...
주문서형·결제창형 연동 키(`test_gck_` / `test_gsk_`)를 쓴다. `toss-pay.html`은 widgets 주문서형(`renderPaymentMethods` · `renderAgreement` · 결제하기)이다. API 개별 연동(`test_ck_` / `test_sk_`)은 결제창(구버전)이라 맞지 않는다.
