...
| Path | 화면 |
|------|------|
| `/` | **대표 상품** 카탈로그. 손님 입구는 게스트 1차 15개 → L2 → 축([guest-l1.md](./guest-l1.md)). Browse 쿼리: `/`, `/?l1=…`, `/?l1=…&l2=…`, `/?l1=…&l2=…&axis=brand|menu|seller`, `/?l1=…&l2=…&brand=` / `&menu=`, `/?q=` |
...
- 공식·입점 혼합 장바구니·결제 (원 스텁; 실 PG는 Phase 4)
...
| 2 | **원 지갑 동시성** — 테스트 스텁 지갑 잠금 | **Phase 4로 이관** — 실제 결제 중복·상태 정합성으로 대체 |
...
- 상세 담기: 수량 스테퍼, 재고·품절 반영. **적용됨**
- 게스트 담기: 로그인 없이 게스트 장바구니(웹 localStorage). 로그인 시 같은 오퍼는 한 줄로 합친다. 주문·토스 준비·주문 생성은 로그인. 헤더 로그인도 `next` 유지. **적용됨**
- 담기 후 스낵바 「장바구니 보기」 + 헤더·탭 장바구니 수량 뱃지. **적용됨**
- 바로구매 없음. 체크아웃은 수량 변경 불가·토스 **웹 전용**.
