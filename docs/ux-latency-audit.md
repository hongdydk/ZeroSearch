...
조사 시점: `main` + 이 브랜치. 판매자·관리자는 **감사만** (동작 변경 없음).  
...
- **구매자 장바구니(이전 후속):** 항목 1·2·4 + 게스트 로컬 카트. `CartNotifier`가 +/-·삭제를 즉시 그리고, 구매자만 debounce PUT/DELETE 후 실패 시 롤백·스낵바. `/cart` 공개, `/checkout` 인증 게이트 유지.
- **판매자·관리자:** 인벤토리만. **수정하지 않음.**
...
- **로그인:** POST+GET+버튼 잠금. (이 PR: 게스트 담기는 로컬 카트. 로그인 담기는 아직 await POST.)
...
게스트 **N/A**. 역할: 관리자 JWT + `user.isAdmin`. `AdminScreen`이 홈·통계·입점·주문·카탈로그·사용자·도구를 **한 클래스로** 갖고, 라우트마다 `section`만 다름. 알림·저장 필터·감사 로그는 Coming Soon.
...
2. 로그인 **담기** 뱃지/목록 (med) — 게스트 담기는 이 PR에서 로컬.
3. **배송지 삭제** 낙관적 + skipLoading (med)
4. 검색 첫 입력 때 랜딩을 스피너로 바꾸지 않기 (med)

판매자 (감사만, Phase 3와 섞지 말고 고칠 때도 이 화면 최소 diff)

1. **S3 주문 상태 탭** — 로컬 전진 + PATCH, silent GET (high)
2. **S6 오퍼 `_load` 스피너** — 이전 목록 유지. 초안 제출 후 prepend (high)
3. **S1 홈 직렬 fetch** — 병렬 또는 요약 (med)
4. S7 품목 검색 다이얼로그 스피너 (med)
5. S4 폴링 가시 범위 (low)

관리자 (감사만)

1. **A1·A2 섹션 `_load` / 거짓 빈 화면** — 섹션별 fetch + 이전 본문 (high, 공수 중간)
2. **A3 입점 승인** — 줄 제거 후 POST, 전체 `_load` 금지 (high)
3. **A9 사용자 검색** — `adminUsers`만 (high)
4. **A4 주문 보정** — S3와 동일 (high)
5. A6 초안 큐에서 한 줄 제거 (med). 승격은 서버 확인 유지
6. A5 주문 폴링을 주문 섹션으로 제한 (med, 매우 작음)
7. A7 카드 검색 다이얼로그 (med)
8. A8·A11 유지

판매자·관리자 수정은 **별도 Plan** 승인 후. 이 문서는 우선순위용 목록이다.
