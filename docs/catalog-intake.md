...
- 카드 아래에 옵션을 여러 개 둔다. 각 옵션은 이름(예: 맛·종류), 수량과 단위(`ml`, `L`, `g`, `kg`, `팩`), 팩 수, 선택 사진을 가진다.
- 판매자는 새 카드 또는 기존 카드에 옵션 여러 개를 한 번에 제안한다. MD는 같은 검수 흐름에서 기존 카드에 붙이거나 새 카드로 승격한다. 관리자는 카드 생성이나 기존 카드 편집 시 옵션 여러 개를 직접 추가한다.
- 옵션 제안만으로 판매 오퍼는 생성하지 않는다. 판매자가 승인된 옵션에 가격·재고를 등록하고 검수를 거쳐야 구매자에게 표시된다.
- 상세의 판매자 비교는 선택한 옵션에 연결된 공개 오퍼를 기준으로 한다. 이전에 등록한 옵션 연결 없는 오퍼는 별도 선택지로 유지한다.
- 관리자 카탈로그 CSV는 `제조사, 카드명, 종류, 카드설명, 카드사진URL, 옵션명, 수량, 단위, 팩수, 옵션사진URL` 열을 사용한다. 한 카드의 옵션은 여러 행으로 입력한다. 포털에서 현재 카탈로그 CSV와 빈 템플릿 CSV를 각각 내려받을 수 있다.
