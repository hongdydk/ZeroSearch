...
**증상 (당시):** 상세에서 비로그인 담기 → 로그인 화면으로 간 뒤, 로그인 성공해도 홈(`/`)으로 가고 상세·담기 흐름이 끊긴다.

**원인 (당시):** `catalog_detail_screen.dart`가 `go('/login')`만 호출하고 `next`(복귀 URL)를 넘기지 않는다. 로그인 성공 리다이렉트도 `/` 고정.

**현재 규칙:** 게스트 담기는 로그인으로 보내지 않는다. 선택한 수량으로 게스트 장바구니에 넣고 새로고침 뒤에도 유지한다. 같은 오퍼는 한 줄. 로그인하면 사용자 장바구니에 합친다. 주문하기·토스 준비·주문 생성에서만 로그인(`next`는 체크아웃 또는 장바구니).
