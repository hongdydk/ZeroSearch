...
- **결제:** Phase 4 Flutter 웹 토스 카드 테스트 결제. 라이브 키·취소/환불은 추후. Phase 3은 판매자·관리자 기능 추가.
- **배포:** Cloudflare Pages(Flutter, same-origin `/api`) + EC2(FastAPI·Postgres). 카탈로그는 `data/aihub-catalog.csv` 배포 시 import.
